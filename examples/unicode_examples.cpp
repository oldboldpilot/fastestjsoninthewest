...
void example_utf16_surrogate_pairs() {
    ...
    for (const auto& ex : examples) {
        ...
        std::cout << "    UTF-32 Codepoint: U+" << std::hex << std::setfill('0') 
                  << std::setw(4) << ex.codepoint << std::dec << "\n";
        ...
        if (cp >= 0x10000) {
            ...
            std::cout << "    UTF-16 High: 0x" << std::hex << high 
                      << " Low: 0x" << low << std::dec << "\n";
...
void example_utf32_codepoints() {
    ...
    std::cout << std::left << std::setw(15) << "Code Point" 
              << std::setw(40) << "Name" 
              << std::setw(5) << "Char" << "\n";
    ...
    for (const auto& cp : codepoints) {
        ...
        std::cout << "U+" << std::hex << std::setfill('0') << std::setw(6) << cp.value 
                  << std::dec << std::setfill(' ')
                  << std::setw(40) << cp.name 
                  << std::setw(5) << cp.utf8;
