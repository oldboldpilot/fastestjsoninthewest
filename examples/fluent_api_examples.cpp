...
void example_text_processing() {
    ...
    std::cout << "Words starting with '" << prefix << "' and length >= " << min_length << ":" << std::endl;
    from(words)
        .filter([prefix](const std::string& w) { 
            return !w.empty() && w[0] == prefix; 
        })
        ...
        .forEach([](const std::string& w) {
            std::cout << "  - " << w << std::endl;
        });
    
    // Word length statistics
    auto lengths = from(words)
        .map([](const std::string& w) { return w.length(); });
...
void example_zip_operations() {
    ...
    from(names)
        .zip(from(scores), [](const std::string& name, int score) {
            return std::make_pair(name, score);
        })
        .zip(from(grades), [](const auto& pair, const std::string& grade) {
    ...
    auto weighted_sum = from(scores)
        .zip(from(weights), [](int score, double weight) {
            return score * weight;
        })
        .reduce(0.0, [](double acc, double val) { return acc + val; });
    
    std::cout << "\nWeighted Average Score: " << std::fixed << std::setprecision(2) 
...
void example_find_operations() {
    ...
    auto found_user = from(users)
        .find([](const json_object& user) {
    ...
    int target_id = 3;
    auto user_index = from(users)
        .find_index([target_id](const json_object& user) {
            return user.at("id").as_number() == target_id;
        });
    
    if (user_index.has_value()) {
        std::cout << "User with ID " << target_id << " is at index " << user_index.value() << std::endl;
    }
    
    // Check if any inactive users exist
    bool has_inactive = from(users)
        .any([](const json_object& user) {
            return !user.at("active").as_bool();
        });
    ...
    from(users)
        .filter([](const json_object& user) {
            return user.at("active").as_bool();
        })
        .map([](const json_object& user) {
            return user.at("email").as_string();
        })
        .forEach([](const std::string& email) {
...
void example_nested_chains() {
    ...
    std::cout << "Pipeline: numbers → filter(even) → map(square) → filter(>20) → map(+100) → sort(desc) → take(3)" << std::endl;
    ...
    auto result = from(numbers)
        ...
        .order_by_descending([](int n) { 
            return n; 
        })
        .take(3);
    
    std::cout << "Results: ";
    result.forEach([](int n) {
        std::cout << n << " ";
    });
    std::cout << std::endl;
