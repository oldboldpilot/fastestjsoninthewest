...
public:
    ...
    query_result<T> where(Predicate pred) const {
        std::vector<T> result;
    ...
    template<typename Predicate>
    query_result<T> filter(Predicate pred) const {
        return where(pred);
    }
    ...
    query_result<T> order_by(KeySelector key_selector) const {
    ...
    std::optional<T> first_or_default(const T& default_value) const {
        if (data_.empty()) return default_value;
        return data_.front();
    }
    
    template<typename Predicate>
    std::optional<T> first(Predicate pred) const {
        for (const auto& item : data_) {
    ...
    template<typename Predicate>
    std::optional<T> last(Predicate pred) const {
    ...
    template<typename Predicate>
    std::optional<T> find(Predicate pred) const {
    ...
    template<typename Predicate>
    std::optional<size_t> find_index(Predicate pred) const {
    ...
    template<typename Func>
    query_result<T> prefix_sum(Func func) const {
        if (data_.empty()) return query_result<T>();
    ...
    template<typename Func>
    query_result<T> scan(Func func) const {
        return prefix_sum(func);
    }
    
    query_result<T> scan() const {
        return prefix_sum();
    }
    ...
    template<typename Func>
    std::optional<T> aggregate(Func func) const {
        if (data_.empty()) return std::nullopt;
    ...
    template<typename Func>
    std::optional<T> reduce(Func func) const {
        return aggregate(func);
    }
    
    template<typename Acc, typename Func>
    Acc reduce(Acc seed, Func func) const {
    ...
    template<typename Selector>
    auto average(Selector selector) const -> std::optional<double> {
        if (data_.empty()) return std::nullopt;
        using R = decltype(selector(std::declval<T>()));
    ...
    query_result<T> distinct() const {
        std::set<T> unique(data_.begin(), data_.end());
        std::vector<T> result(unique.begin(), unique.end());
        return query_result<T>(std::move(result));
    }
    
    template<typename KeySelector>
    query_result<T> distinct_by(KeySelector key_selector) const {
        std::vector<T> result;
        std::set<decltype(key_selector(std::declval<T>()))> seen;
        for (const auto& item : data_) {
    ...
    template<typename Predicate>
    query_result<T> take_while(Predicate pred) const {
        std::vector<T> result;
        for (const auto& item : data_) {
            if (!pred(item)) break;
            result.push_back(item);
    ...
    template<typename Predicate>
    query_result<T> skip_while(Predicate pred) const {
        std::vector<T> result;
        bool skipping = true;
        for (const auto& item : data_) {
    ...
    auto group_by(KeySelector key_selector) const 
        -> std::map<decltype(key_selector(std::declval<T>())), std::vector<T>> {
    ...
    template<typename TOther, typename KeySelector1, typename KeySelector2, typename ResultSelector>
    auto join(const query_result<TOther>& other,
              KeySelector1 key_selector1,
              KeySelector2 key_selector2,
              ResultSelector result_selector) const
    ...
    auto zip(const query_result<TOther>& other) const
    ...
    query_result<T> intersect(const query_result<T>& other) const {
        std::set<T> set1(data_.begin(), data_.end());
        std::set<T> set2(other.begin(), other.end());
        std::vector<T> result;
...
public:
    ...
    // Parallel WHERE
    template<typename Predicate>
    parallel_query_result<T> where(Predicate pred) const {
    ...
    // Alias: filter (functional programming style)
    template<typename Predicate>
    parallel_query_result<T> filter(Predicate pred) const {
        return where(pred);
    }
    ...
    template<typename Func>
    parallel_query_result<T> prefix_sum(Func func) const {
        if (data_.empty()) return parallel_query_result<T>(std::vector<T>());
        if (data_.size() == 1) return parallel_query_result<T>(std::vector<T>(data_));
    ...
    // Alias: scan (functional programming style)
    template<typename Func>
    parallel_query_result<T> scan(Func func) const {
        return prefix_sum(func);
    }
    
    parallel_query_result<T> scan() const {
        return prefix_sum();
    }
    ...
    query_result<T> as_sequential() const {
...
template<typename T>
query_result<T> from(const std::vector<T>& vec) {
    return query_result<T>(std::vector<T>(vec));
}

template<typename T>
parallel_query_result<T> from_parallel(const std::vector<T>& vec) {
