...
struct numa_functions {
    ...
    int (*get_mempolicy)(int* mode, unsigned long* nodemask, unsigned long maxnode, void* addr,
...
static auto init_numa_library() -> bool {
    ...
    #define LOAD_FUNC(name) \
        g_numa_funcs.name = \
            reinterpret_cast<decltype(g_numa_funcs.name)>(dlsym(g_numa_lib_handle, #name))
