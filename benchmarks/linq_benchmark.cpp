...
void benchmark_filter(size_t count) {
    ...
    auto linq_result =
        from(objects)
            .where([](const json_object& obj) { return obj.at("value").as_number() > 500; })
            .to_vector();
    ...
    t.start();
    auto plinq_result =
        from_parallel(objects)
            .where([](const json_object& obj) { return obj.at("value").as_number() > 500; })
            .to_vector();
    double plinq_time = t.elapsed_ms();

    std::cout << std::fixed << std::setprecision(2);
    std::cout << "  Manual loop:     " << std::setw(10) << manual_time << " ms (baseline)"
              << std::endl;
    ...
    std::cout << "  Results match: " << (linq_result.size() == manual_result.size() ? "✓" : "✗")
              << std::endl;
}
...
void benchmark_transform(size_t count) {
    ...
    auto linq_result =
        from(objects)
            .select([](const json_object& obj) { return obj.at("value").as_number() * 2.0; })
            .to_vector();
    double linq_time = t.elapsed_ms();
...
void benchmark_aggregate(size_t count) {
    ...
    t.start();
    auto linq_result =
        from(objects).sum([](const json_object& obj) { return obj.at("value").as_number(); });
    double linq_time = t.elapsed_ms();

    // LINQ parallel
    t.start();
    auto plinq_result = from_parallel(objects).sum(
        [](const json_object& obj) { return obj.at("value").as_number(); });
    double plinq_time = t.elapsed_ms();
    ...
    std::cout << "  LINQ parallel:   " << std::setw(10) << plinq_time << " ms ("
              << (plinq_time / manual_time) << "x)" << std::endl;
    std::cout << "  Results match: " << (std::abs(linq_result - manual_result) < 0.01 ? "✓" : "✗")
              << std::endl;
}
...
void benchmark_any(size_t count) {
    ...
    t.start();
    auto linq_result =
        from(objects).any([](const json_object& obj) { return obj.at("value").as_number() > 900; });
    double linq_time = t.elapsed_ms();

    // LINQ parallel
    t.start();
    auto plinq_result = from_parallel(objects).any(
        [](const json_object& obj) { return obj.at("value").as_number() > 900; });
    double plinq_time = t.elapsed_ms();
