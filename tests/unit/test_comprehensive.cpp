...
#define TEST(name, condition)                                                           \
    do {                                                                                \
        if (condition) {                                                                \
            std::cout << "✓ Test " << (tests_passed + tests_failed + 1) << ": " << name \
                      << " PASSED\n";                                                   \
            tests_passed++;                                                             \
        } else {                                                                        \
            std::cout << "✗ Test " << (tests_passed + tests_failed + 1) << ": " << name \
...
int main() {
    ...
    TEST("Parse float",
         r5.has_value() && r5->is_number() && std::abs(r5->as_number() - 3.14159) < 0.00001);
    ...
    TEST("String with space",
         s3.has_value() && s3->is_string() && s3->as_string() == "hello world");
    ...
    TEST("Negative decimal with negative exp",
         n9.has_value() && std::abs(n9->as_number() + 0.015) < 0.0001);
    ...
    TEST("Array with whitespace",
         a10.has_value() && a10->is_array() && a10->as_array().size() == 3);
    ...
    TEST("Single property object",
         o2.has_value() && o2->is_object() && o2->as_object().size() == 1);
