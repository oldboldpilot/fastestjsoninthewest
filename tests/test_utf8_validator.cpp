...
int main() {
    ...
    bool valid = fastjson::validate_utf8_scalar(
        std::span<const char>(test1.data(), test1.size()), 0, test1.size());
    ...
    valid = fastjson::validate_utf8_scalar(
        std::span<const char>(test2.data(), test2.size()), 0, test2.size());
