...
void test_map_filter_reduce() {
    ...
    auto sum = from(numbers)
        .reduce(0, [](int acc, int n) { return acc + n; });
    assert(sum == 55);
    std::cout << "✓ reduce: sum = " << sum << std::endl;
    
    // REDUCE - functional style (product)
    auto product = from(numbers)
        .filter([](int n) { return n <= 5; })
        .reduce(1, [](int acc, int n) { return acc * n; });
...
void test_zip() {
    ...
    auto pairs = from(numbers)
        .zip(from(words), [](int n, const std::string& s) {
    ...
    auto simple_pairs = from(numbers)
        .zip(from(words))
        .to_vector();
    ...
    auto truncated = from(numbers)
        .zip(from(short_list), [](int a, int b) { return a + b; })
        .to_vector();
...
void test_foreach() {
    ...
    int sum = 0;
    from(numbers).forEach([&sum](int n) {
        sum += n;
    });
    assert(sum == 15);
...
void test_closures() {
    ...
    // Capture by value
    int threshold = 3;
    auto filtered = from(numbers)
        .filter([threshold](int n) { return n > threshold; })
        .to_vector();
    assert(filtered.size() == 2);
    std::cout << "✓ Closure [value]: filtered " << filtered.size() << " items > " << threshold << std::endl;
    
    // Capture by reference
    int multiplier = 2;
    auto mapped = from(numbers)
        .map([&multiplier](int n) { return n * multiplier; })
        .to_vector();
    ...
    int min_val = 2;
    int max_val = 4;
    auto range_filtered = from(numbers)
        .filter([min_val, max_val](int n) { 
            return n >= min_val && n <= max_val; 
        })
        .to_vector();
    assert(range_filtered.size() == 3);  // {2, 3, 4}
    std::cout << "✓ Closure [multi]: filtered range [" << min_val << ", " << max_val << "]" << std::endl;
...
void test_parallel_functional() {
    ...
    // Parallel filter
    auto evens = from_parallel(large_data)
        .filter([](int n) { return n % 2 == 0; })
        .to_vector();
    ...
    // Parallel map
    auto squared = from_parallel(large_data)
        .map([](int n) { return n * n; })
        .to_vector();
...
void test_json_functional() {
    ...
    auto high_scorers = from(users)
        .filter([](const json_object& u) {
            return u.at("score").as_number() >= 90;
        })
        .map([](const json_object& u) {
            return u.at("name").as_string();
        })
        .to_vector();
    ...
    auto found = from(users)
        .find([](const json_object& u) {
            return u.at("score").as_number() > 90;
        });
    ...
    auto total_score = from(users)
        .reduce(0.0, [](double acc, const json_object& u) {
            return acc + u.at("score").as_number();
        });
    ...
    auto ranked = from(users)
        .zip(from(ranks), [](const json_object& u, int rank) {
    ...
    std::cout << "  All users: ";
    from(users).forEach([](const json_object& u) {
        std::cout << u.at("name").as_string() << " ";
    });
    std::cout << std::endl;
