...
void test_whitespace_handling() {
    ...
    for (size_t i = 0; i < sizeof(patterns) / sizeof(patterns[0]); ++i) {
        ...
        print_result(("Whitespace pattern " + std::to_string(i + 1)).c_str(), 
                    result.has_value());
    }
}
...
void test_string_scanning() {
    ...
    test_strings.push_back(R"({"s":"This is a much longer test string that should exercise the SIMD code paths."})");
    
    // Very long strings (> 64 bytes)
    test_strings.push_back(R"({"s":"This is an even longer string designed to test the NEON SIMD implementation with strings that span multiple 16-byte chunks to ensure proper handling."})");
    
    for (size_t i = 0; i < test_strings.size(); ++i) {
        auto result = fastjson::parse(test_strings[i]);
        print_result(("String length " + std::to_string(i + 1)).c_str(), 
                    result.has_value());
    }
}
...
void test_utf8_validation() {
    ...
    for (size_t i = 0; i < sizeof(valid) / sizeof(valid[0]); ++i) {
        ...
        print_result(("Valid UTF-8 " + std::to_string(i + 1)).c_str(), 
                    result.has_value());
    }
}
...
void test_error_handling() {
    ...
    for (size_t i = 0; i < sizeof(invalid) / sizeof(invalid[0]); ++i) {
        ...
        print_result(("Error detection " + std::to_string(i + 1)).c_str(), 
                    !result.has_value());
    }
}
