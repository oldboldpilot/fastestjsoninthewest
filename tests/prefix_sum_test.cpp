...
void test_prefix_sum_with_max() {
    ...
    auto result = from(numbers)
        .scan([](int a, int b) { return std::max(a, b); })
        .to_vector();
...
void test_prefix_sum_with_seed() {
    ...
    std::vector<int> numbers = {1, 2, 3, 4, 5};
    auto result = from(numbers)
        .prefix_sum(100, [](int a, int b) { return a + b; })
        .to_vector();
...
void test_prefix_sum_string_concatenation() {
    ...
    auto result = from(words)
        .prefix_sum([](const std::string& a, const std::string& b) { 
            return a + " " + b; 
        })
        .to_vector();
...
void test_parallel_prefix_sum() {
    ...
    std::cout << "✓ Parallel prefix sum on 10,000 elements: last value = " << result.back() << std::endl;
}
...
void test_real_world_sales_cumulative() {
    ...
    auto cumulative_revenue = from(sales)
        .select([](const Sale& s) { return s.revenue; })
        .prefix_sum()
        .to_vector();
