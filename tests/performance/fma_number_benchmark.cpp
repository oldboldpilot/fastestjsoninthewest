...
auto main() -> int {
    ...
    std::cout << "Average: " << (duration_fma / static_cast<double>(iterations))
              << " ms per parse\n";
    std::cout << "Throughput: "
              << (json.size() * iterations / 1024.0 / 1024.0) / (duration_fma / 1000.0)
              << " MB/s\n\n";
    ...
    std::cout << "Time (no FMA): " << duration_no_fma << " ms for " << iterations
              << " iterations\n";
    std::cout << "Average: " << (duration_no_fma / static_cast<double>(iterations))
              << " ms per parse\n";
    std::cout << "Throughput: "
              << (json.size() * iterations / 1024.0 / 1024.0) / (duration_no_fma / 1000.0)
              << " MB/s\n\n";
    ...
    std::cout << "Improvement: " << std::fixed << std::setprecision(1) << ((speedup - 1.0) * 100)
              << "%\n";
    std::cout << "========================================\n";
