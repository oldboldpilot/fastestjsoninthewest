...
public:
    static auto generate_large_json_file(const std::string& filename, size_t target_size_mb)
        -> void {
        ...
        while (current_size < target_bytes) {
            ...
            file << "      \"nested\": {\n";
            file << "        \"coordinates\": [" << real_dist(gen) << ", " << real_dist(gen) << ", "
                 << real_dist(gen) << "],\n";
            file << "        \"metadata\": {\n";
...
// OpenMP configuration
        ...
        for (auto thread_count : thread_counts) {
            ...
            if (!parse_times.empty()) {
                auto avg_time = std::accumulate(parse_times.begin(), parse_times.end(), 0.0)
                                / parse_times.size();
                auto throughput_mb_s = (file_size / (1024.0 * 1024.0)) / (avg_time / 1000.0);

                std::cout << "Average time: " << std::fixed << std::setprecision(2) << avg_time
                          << " ms\n";
                std::cout << "Throughput: " << std::fixed << std::setprecision(2) << throughput_mb_s
                          << " MB/s\n";
            }
        }
    }
