...
private:
    ...
    auto benchmark_string_scan_sse42(const char* data, size_t length) -> OperationResult {
        ...
        for (int iter = 0; iter < iterations; ++iter) {
            ...
            for (; i + 16 <= length; i += 16) {
                ...
                int result =
                    _mm_cmpestri(quote_set, 1, data_vec, 16, _SIDD_CMP_EQUAL_EACH | _SIDD_BIT_MASK);
                if (result < 16) {
                    quote_count++;
                }
            }
    ...
    __attribute__((target("avx512f,avx512bw"))) auto benchmark_string_scan_avx512(const char* data,
                                                                                  size_t length)
        -> OperationResult {
        const int iterations = 1000;
    ...
    auto benchmark_number_parse_sse2(const std::vector<std::string>& numbers) -> OperationResult {
        ...
        result.operations_per_second =
            static_cast<size_t>(numbers.size() * 1e9 / result.time_nanoseconds);
        return result;
    }
    ...
    auto print_operation_results(const std::vector<OperationResult>& results,
                                 const std::string& category) -> void {
        std::cout << category << " Results:\n";
        std::cout << std::setw(20) << "Instruction Set" << std::setw(15) << "Time (ns)"
                  << std::setw(20) << "Operations/sec" << std::setw(15) << "Efficiency"
                  << std::setw(12) << "Speedup"
                  << "\n";
        ...
        for (const auto& result : results) {
            ...
            std::cout << std::setw(20) << result.instruction_set << std::setw(15) << std::fixed
                      << std::setprecision(0) << result.time_nanoseconds << std::setw(20)
                      << result.operations_per_second << std::setw(15) << std::fixed
                      << std::setprecision(6) << result.efficiency_score << std::setw(12)
