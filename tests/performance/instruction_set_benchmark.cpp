...
public:
    ...
    static auto detect() -> uint64_t {
        ...
        if (eax >= 1) {
            ...
            if (edx & (1 << 25)) caps |= SSE;
            if (edx & (1 << 26)) caps |= SSE2;
            if (ecx & (1 << 0))  caps |= SSE3;
            if (ecx & (1 << 9))  caps |= SSSE3;
            if (ecx & (1 << 19)) caps |= SSE41;
            if (ecx & (1 << 20)) caps |= SSE42;
            if (ecx & (1 << 28)) caps |= AVX;
            if (ecx & (1 << 12)) caps |= FMA;
        }
        
        if (eax >= 7) {
            __cpuid_count(7, 0, eax, ebx, ecx, edx);
            if (ebx & (1 << 5))  caps |= AVX2;
            if (ebx & (1 << 16)) caps |= AVX512F;
            if (ebx & (1 << 30)) caps |= AVX512BW;
            if (ebx & (1 << 28)) caps |= AVX512CD;
            if (ebx & (1 << 17)) caps |= AVX512DQ;
            if (ebx & (1 << 31)) caps |= AVX512VL;
            if (ecx & (1 << 11)) caps |= VNNI;
            if (edx & (1 << 24)) caps |= AMX_TILE;
            if (edx & (1 << 25)) caps |= AMX_INT8;
            if (edx & (1 << 22)) caps |= AMX_BF16;
        }
#endif
    ...
    static auto to_string(uint64_t caps) -> std::vector<std::string> {
        ...
        if (caps & SSE)      result.push_back("SSE");
        if (caps & SSE2)     result.push_back("SSE2");
        if (caps & SSE3)     result.push_back("SSE3");
        if (caps & SSSE3)    result.push_back("SSSE3");
        if (caps & SSE41)    result.push_back("SSE4.1");
        if (caps & SSE42)    result.push_back("SSE4.2");
        if (caps & AVX)      result.push_back("AVX");
        if (caps & AVX2)     result.push_back("AVX2");
        if (caps & AVX512F)  result.push_back("AVX-512F");
        if (caps & AVX512BW) result.push_back("AVX-512BW");
        if (caps & AVX512CD) result.push_back("AVX-512CD");
        if (caps & AVX512DQ) result.push_back("AVX-512DQ");
        if (caps & AVX512VL) result.push_back("AVX-512VL");
        if (caps & FMA)      result.push_back("FMA");
        if (caps & FMA4)     result.push_back("FMA4");
        if (caps & VNNI)     result.push_back("VNNI");
        if (caps & AMX_TILE) result.push_back("AMX-TILE");
        if (caps & AMX_INT8) result.push_back("AMX-INT8");
        if (caps & AMX_BF16) result.push_back("AMX-BF16");
        if (caps & NEON)     result.push_back("NEON");
...
struct BenchmarkResult {
    ...
    auto print() const -> void {
        std::cout << std::setw(20) << name
                  << std::setw(15) << instruction_set
                  << std::setw(12) << std::fixed << std::setprecision(3) << parse_time_ms
                  << std::setw(15) << std::fixed << std::setprecision(2) << throughput_mbps
                  << std::setw(12) << operations_per_second
                  << std::setw(10) << std::fixed << std::setprecision(2) << speedup_vs_scalar
                  << std::setw(8) << (openmp_enabled ? "Yes" : "No")
                  << std::setw(8) << thread_count
                  << std::setw(12) << memory_usage_mb
                  << "\n";
    }
};
...
public:
    ComprehensiveSIMDBenchmark(size_t test_size_mb = 10) 
        : test_size_mb_(test_size_mb) {
...
private:
    auto generate_test_data() -> void {
        ...
        json_stream << "    \"timestamp\": " << duration_cast<seconds>(system_clock::now().time_since_epoch()).count() << "\n";
        json_stream << "  },\n";
        json_stream << "  \"data\": [\n";
    ...
    auto benchmark_with_config(const std::string& name, bool force_scalar = false, int thread_count = 1) -> BenchmarkResult {
    ...
    auto get_active_instruction_set() -> std::string {
        ...
        if (capabilities_ & SIMDCapabilities::AVX512F) return "AVX-512";
        if (capabilities_ & SIMDCapabilities::AVX2) return "AVX2";
        if (capabilities_ & SIMDCapabilities::AVX) return "AVX";
        if (capabilities_ & SIMDCapabilities::SSE42) return "SSE4.2";
        if (capabilities_ & SIMDCapabilities::SSE2) return "SSE2";
        return "Scalar";
    }
...
public:
    auto run_comprehensive_benchmark() -> std::vector<BenchmarkResult> {
        ...
        for (int threads : {2, 4, 8, max_threads}) {
            if (threads <= max_threads) {
                auto openmp_result = benchmark_with_config(
                    "SIMD+OpenMP(" + std::to_string(threads) + ")", 
                    false, 
                    threads
                );
        ...
        auto best_result = *std::min_element(results.begin(), results.end(), 
            [](const auto& a, const auto& b) { return a.parse_time_ms < b.parse_time_ms; });
...
int main(int argc, char* argv[]) {
    try {
        ...
        for (size_t i = 0; i < results.size(); ++i) {
            ...
            results_file << "      \"openmp_enabled\": " << (results[i].openmp_enabled ? "true" : "false") << ",\n";
