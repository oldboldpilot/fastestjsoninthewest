...
struct benchmark_result {
    ...
    void print() const {
        std::cout << std::setw(20) << execution_mode << std::setw(8) << num_threads << std::setw(8)
                  << (simd_enabled ? "Yes" : "No") << std::setw(12) << std::fixed
                  << std::setprecision(2) << parse_time_ms << std::setw(12) << std::fixed
                  << std::setprecision(2) << throughput_mbs << std::setw(10) << std::fixed
                  << std::setprecision(2) << speedup_vs_baseline << "x" << std::setw(10)
                  << std::fixed << std::setprecision(1) << (parallel_efficiency * 100) << "%"
                  << "\n";
    }
};
...
auto benchmark_parse(const std::string& json_data, int num_threads, bool enable_simd)
    -> benchmark_result {
...
void test_openmp_scaling(const std::string& json_data, const std::vector<int>& thread_counts) {
    ...
    std::cout << std::setw(20) << "Execution Mode" << std::setw(8) << "Threads" << std::setw(8)
              << "SIMD" << std::setw(12) << "Time (ms)" << std::setw(12) << "MB/s" << std::setw(10)
              << "Speedup" << std::setw(10) << "Efficiency"
              << "\n";
    ...
    auto best = std::max_element(results.begin(), results.end(), [](const auto& a, const auto& b) {
        return a.speedup_vs_baseline < b.speedup_vs_baseline;
    });

    if (best != results.end()) {
        std::cout << "Best configuration: " << best->execution_mode << " (" << best->num_threads
                  << " threads, " << (best->simd_enabled ? "SIMD enabled" : "SIMD disabled")
                  << ")\n";
