...
auto benchmark_config(const std::string& test_name, const std::string& json,
                      const fastjson::parse_config& config, int iterations = 5) -> void {
...
#pragma omp parallel for reduction(+ : sum) if (config.enable_parallel)
    ...
    std::cout << std::setw(30) << std::left << test_name << ": " << std::fixed
              << std::setprecision(2) << avg_time << " ms (min: " << min_time
              << ", max: " << max_time << ") "
              << "| " << throughput << " MB/s\n";
}
