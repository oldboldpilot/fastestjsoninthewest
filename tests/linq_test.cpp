...
void test_parallel_linq() {
    ...
    // Parallel WHERE
    auto evens = from_parallel(numbers)
        .where([](int n) { return n % 2 == 0; })
        .to_vector();
    ...
    // Parallel SELECT
    auto doubled = from_parallel(numbers)
        .select([](int n) { return n * 2; })
        .to_vector();
...
void test_json_linq() {
    ...
    // Find oldest
    auto oldest = from(objects)
        .order_by_descending([](const json_object& obj) {
            return obj.at("age").as_number();
        })
        .first();
